...
def multilook(array, nlook, n_valid_thre=0.5):
    ''' to just simple-multilook real numbers'''
...
def multilook_w(array, nlook, n_valid_thre=0.5):
    ...
    with warnings.catch_warnings(): ## To silence RuntimeWarning: Mean of empty slice
        warnings.simplefilter('ignore', RuntimeWarning)
        temp=np.exp(1j*array_reshape)
        array_ml =np.angle(np.nansum(temp, axis=(1,3)) )
